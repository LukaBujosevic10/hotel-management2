...
public class GlobalExceptionHandler {
    ...
    /**
     * A downstream service (guest-service / room-service) answered with an error.
     * Translate it into something the end user understands instead of leaking
     * the raw Feign stack trace.
     */
    @ExceptionHandler(FeignException.class)
    public ResponseEntity<ApiError> handleFeign(FeignException ex, HttpServletRequest req) {
        log.warn("Downstream call failed on {}: status={} {}", req.getRequestURI(), ex.status(), ex.getMessage());
        if (ex.status() == 404) {
            return build(HttpStatus.BAD_REQUEST,
                    "A referenced guest or room does not exist any more.", req, null);
        }
        return build(HttpStatus.SERVICE_UNAVAILABLE,
                "Another service is currently unreachable, so the request could not be completed. "
                        + "Please try again in a moment.", req, null);
    }
